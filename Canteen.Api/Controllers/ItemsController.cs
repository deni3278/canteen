...
public class ItemsController : ControllerBase
{
    ...
    public async Task<IActionResult> GetItemsAsync(bool active, [FromQuery] bool withImage = true)
    {
        ...
        if (itemResult == null!) return StatusCode(StatusCodes.Status500InternalServerError);
    ...
    {
        var item = await _context.Items.ProjectTo<ItemDto>(_mapper.ConfigurationProvider).FirstAsync(dto => dto.ItemId == id);
    ...
    public async Task<IActionResult> CreateItemAsync(ItemDto itemDto)
    {
        ...
        if (item == null) return StatusCode(StatusCodes.Status500InternalServerError);
    ...
    {
        var images = await _context.Items.Where(item => ids.Contains(item.ItemId)).Select(item => item.Image).ToListAsync();
